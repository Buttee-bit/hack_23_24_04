...
const BadCategories: FC<Props> = ({
	...
	const handleCategory = (text: string) => {
		...
		if (!inAltArray) {
			...
			toast(
				'Одна из выбранных категорий уже в списке желаемых категорий!'
			)
		}
	}
	...
	const handleAddAll = (values: string[]) => {
		values.forEach(text => {
			...
			if (!inAltArray) {
				...
				toast(
					'Некоторые категории не были добавлены в список, поскольку они указаны в желаемых категориях!'
				)
			}
		})
	}
